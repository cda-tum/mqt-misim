...
[[maybe_unused]] static std::ostream &modernNode(const Package::mEdge &e,
                                                 std::ostream &os,
                                                 bool formatAsPolar = true) {
...
[[maybe_unused]] static std::ostream &modernNode(const Package::vEdge &e,
                                                 std::ostream &os,
                                                 bool formatAsPolar = true) {
  auto nodelabel = (reinterpret_cast<std::uintptr_t>(e.p) & 0x001fffffU) >>
                   1U;  // this allows for 2^20 (roughly 1e6) unique nodes
...
[[maybe_unused]] static std::ostream &classicNode(const Package::mEdge &e,
                                                  std::ostream &os,
                                                  bool formatAsPolar = true) {
  auto nodelabel = (reinterpret_cast<std::uintptr_t>(e.p) & 0x001fffffU) >>
                   1U;  // this allows for 2^20 (roughly 1e6) unique nodes
...
[[maybe_unused]] static std::ostream &classicNode(const Package::vEdge &e,
                                                  std::ostream &os,
                                                  bool formatAsPolar = true) {
  auto nodelabel = (reinterpret_cast<std::uintptr_t>(e.p) & 0x001fffffU) >>
                   1U;  // this allows for 2^20 (roughly 1e6) unique nodes
...
[[maybe_unused]] static std::ostream &bwEdge(const Package::mEdge &from,
                                             const Package::mEdge &to,
                                             short idx, std::ostream &os,
...
[[maybe_unused]] static std::ostream &bwEdge(
    const Package::vEdge &from, const Package::vEdge &to, short idx,
...
[[maybe_unused]] static std::ostream &coloredEdge(const Package::mEdge &from,
                                                  const Package::mEdge &to,
                                                  short idx, std::ostream &os,
                                                  bool edgeLabels = false,
                                                  bool classic = false,
...
[[maybe_unused]] static std::ostream &coloredEdge(
    const Package::vEdge &from, const Package::vEdge &to, short idx,
    std::ostream &os, bool edgeLabels = false,
    [[maybe_unused]] bool classic = false, bool formatAsPolar = true) {
  auto fromlabel =
...
static std::ostream &memoryEdge(const Edge &from, const Edge &to, short idx,
  ...
  if (edgeLabels) {
    ...
    template <class Edge>
    [[maybe_unused]] static void export2Dot(
        Edge basic, const std::string &outputFilename, bool colored = true,
      ...
      if (show) {
        ...
        oss << "dot -Tsvg " << outputFilename << " -o "
            ...
            << ".svg";
        auto str = oss.str();  // required to avoid immediate deallocation of
                               // temporary
    ...
    [[maybe_unused]] static void serialize(const Package::vEdge &basic,
                                           std::ostream &os,
    ...
    static void serializeMatrix(
        const Package::mEdge &basic, std::int_least64_t &idx,
    ...
    [[maybe_unused]] static void serialize(const Package::mEdge &basic,
                                           std::ostream &os,
                                           bool writeBinary = false) {
      if (writeBinary) {
        os.write(reinterpret_cast<const char *>(&SERIALIZATION_VERSION),
